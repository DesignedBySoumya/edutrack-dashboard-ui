...
const PTSReportCard = () => {
  ...
  // Fetch subjects and chapters from Supabase when exam changes
  useEffect(() => {
    const fetchData = async () => {
      const examId = Number(localStorage.getItem('selectedExamId')) || 1;
      const { data: subjects, error: subError } = await supabase.from('subjects').select('*').eq('exam_id', examId);
      if (subError) {
      ...
      for (const subject of subjects) {
        ...
        // Default color logic: never black
  ...
  const calculateAnalytics = () => {
    const totalMarks = Object.values(subjectData).reduce((sum: number, subject: any) => {
      const s = subject as any;
      return sum + s.chapters.reduce((chapterSum: number, chapter: any) => {
        return chapterSum + (Number(chapter.marks) || 0);
      }, 0);
    }, 0);
    const maxPossibleMarks = Object.values(subjectData).reduce((sum: number, subject: any) => sum + subject.maxMarks, 0);
  ...
  const updateChapterData = (subjectKey: string, chapterIndex: number, field: keyof any, value: string) => {
    setSubjectData(prev => ({
      ...prev,
      [subjectKey]: {
        ...prev[subjectKey],
        chapters: prev[subjectKey].chapters.map((chapter: any, idx: number) => idx === chapterIndex ? {
          ...chapter,
          [field]: value
        } : chapter)
      }
  ...
  const calculateSubjectStats = (chapters: any[]) => {
    const totalQuestions = chapters.reduce((sum: number, ch: any) => sum + (Number(ch.correct) || 0) + (Number(ch.incorrect) || 0), 0);
    const attempted = chapters.filter(ch => (Number(ch.correct) || 0) + (Number(ch.incorrect) || 0) > 0).length;
    const totalCorrect = chapters.reduce((sum: number, ch: any) => sum + (Number(ch.correct) || 0), 0);
    const totalIncorrect = chapters.reduce((sum: number, ch: any) => sum + (Number(ch.incorrect) || 0), 0);
    const totalTime = chapters.reduce((sum: number, ch: any) => sum + (Number(ch.timeSpent) || 0), 0);
    const totalMarks = chapters.reduce((sum: number, ch: any) => sum + (Number(ch.marks) || 0), 0);
    return {
      totalQuestions,
      attempted,
  ...
  return <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50">
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="text-center mb-8">
          ...
          <p className="text-purple-600">Enter your performance data and track your progress in real-time</p>
        </div>
        ...
        <div className="space-y-6">
          {Object.entries(subjectData).map(([subjectKey, subject]) => {
            ...
            // In colorMap, ensure no black color is used
            const colorMap = {
            ...
            return <Card key={subjectKey} className={`${colorMap[s.color as keyof typeof colorMap] || colorMap.default} shadow-lg`}>
                    ...
                    <AccordionItem value={subjectKey} className="border-0">
                      ...
                      <AccordionContent className="px-6 pb-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {s.chapters.map((chapter: any, chapterIndex: number) => {
